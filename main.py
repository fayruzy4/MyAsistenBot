...
def handle_gemini_mode(message):
    try:
        ...
        bot.send_message(message.chat.id, ai_mode_status_text("ai_gemini"))
    except Exception as exc:
...
def handle_groq_mode(message):
    try:
        ...
        bot.send_message(message.chat.id, ai_mode_status_text("ai_groq"))
    except Exception as exc:
...
def handle_text(message):
    try:
        ...
        if kind == "ai_gemini":
            ...
            bot.send_message(message.chat.id, escape(answer))
            return
...
def handle_voice(message):
    try:
        ...
        transcript, answer = groq_ai.ask_voice(message.from_user.id, message.voice.file_id, history_limit=16)
    ...
    try:
        ...
        if data == "finance_menu":
            ...
            if gemini_ai is None:
                ...
                bot.send_message(call.message.chat.id, ai_mode_status_text("ai_gemini"))
